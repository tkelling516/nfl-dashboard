# nflverse Play-by-Play Field Dictionary

Full column reference for `nfl_data_py.import_pbp_data()` (nfl_data_py 0.3.3, `include_participation=True`), for use when deciding whether to widen `pbp_plays` beyond its current minimal set of fields.

- **397 total columns** returned by `import_pbp_data()` in this environment.
- **371** are documented in nflfastR's official [field_descriptions](https://nflfastr.com/articles/field_descriptions.html) dictionary (372 fields total there; 1 — `old_game_id` — got merge-suffixed away, see below).
- **26** are NOT in that dictionary — they come from the separate NGS/participation data source that `import_pbp_data()` merges in (formations, personnel, coverage, routes, pressure, etc). Descriptions below for those are inferred from nflverse participation-data conventions, not the official dictionary.
- Columns currently stored in this project's `pbp_plays` table ([db/schema.sql](../db/schema.sql)) are marked ✅.

Source: [nflfastR Field Descriptions](https://nflfastr.com/articles/field_descriptions.html) (Sebastian Carl & Ben Baldwin).

---

## Columns not in the official dictionary (participation/NGS data)

These 26 columns get merged into `import_pbp_data()`'s output from the play participation dataset. **Update:** 6 of them (`route`, `offense_personnel`, `defenders_in_box`, `was_pressure`, `time_to_throw`, `defense_coverage_type`) have since been added to `pbp_plays`.

**Correction on `route`:** it does *not* give a real "routes run" count. `import_pbp_data()` flattens participation data to one row per play, so `route` only ever captures the *targeted* receiver's route on that play — never a non-targeted route-runner's. Verified against real data: a per-receiver/game count of non-null `route` never exceeds `targets` and matches it 96% of the time. So `agg_player_game.routes_run` stays NULL for now rather than being wired to a metric that's just `targets` in disguise. A true routes-run count would need nflverse's raw per-play *multi-player* participation dataset (one row per player per play, not flattened to one row per play), which is a separate, heavier data source nfl_data_py doesn't expose today.

| Column | Description | In current schema |
|---|---|---|
| `old_game_id_x` | Legacy NFL game ID, offense side of a duplicate created when nfl_data_py merges pbp with participation data (both sources carry old_game_id; pandas suffixes the collision). Redundant with game_id. |  |
| `nflverse_game_id` | Canonical nflverse game identifier, distinct from the primary game_id and the legacy old_game_id fields. |  |
| `old_game_id_y` | Legacy NFL game ID, defense side of the same merge-collision duplicate as old_game_id_x. Redundant with game_id. |  |
| `possession_team` | Team in possession, as carried by the participation-data source (duplicate/alias of posteam). |  |
| `offense_formation` | Offensive formation label (e.g. SHOTGUN, SINGLEBACK, I_FORM), from NFL Next Gen Stats participation data. |  |
| `offense_personnel` | Offensive personnel grouping on the play, e.g. '1 RB, 1 TE, 3 WR' (NGS participation data). | ✅ |
| `defenders_in_box` | Count of defenders lined up in the box at snap (NGS participation data). | ✅ |
| `defense_personnel` | Defensive personnel grouping on the play, e.g. '4 DL, 3 LB, 4 DB' (NGS participation data). |  |
| `number_of_pass_rushers` | Count of pass rushers on the play (NGS participation data). |  |
| `players_on_play` | Combined list/count of all players on the field for the play (NGS participation data). |  |
| `offense_players` | List of offensive player IDs on the field for the play (NGS participation data). |  |
| `defense_players` | List of defensive player IDs on the field for the play (NGS participation data). |  |
| `n_offense` | Number of offensive players on the field for the play (NGS participation data). |  |
| `n_defense` | Number of defensive players on the field for the play (NGS participation data). |  |
| `ngs_air_yards` | Air yards as measured by Next Gen Stats tracking (can differ slightly from the official-stats air_yards field). |  |
| `time_to_throw` | Seconds elapsed from snap to throw, per NGS tracking data. | ✅ |
| `was_pressure` | Binary indicator for whether the QB was under pressure on the play, per NGS tracking data. | ✅ |
| `route` | Route type run by the targeted receiver on the play (e.g. SLANT, GO, CROSS), per NGS participation data. Counting non-null rows per receiver/game approximates 'routes run'. | ✅ |
| `defense_man_zone_type` | Whether the defense played man or zone coverage on the play (NGS participation data). |  |
| `defense_coverage_type` | Specific defensive coverage scheme on the play, e.g. Cover 3, Cover 1 (NGS participation data). | ✅ |
| `offense_names` | Player names for the offensive personnel on the field (NGS participation data). |  |
| `defense_names` | Player names for the defensive personnel on the field (NGS participation data). |  |
| `offense_positions` | Positions of the offensive players on the field (NGS participation data). |  |
| `defense_positions` | Positions of the defensive players on the field (NGS participation data). |  |
| `offense_numbers` | Jersey numbers of the offensive players on the field (NGS participation data). |  |
| `defense_numbers` | Jersey numbers of the defensive players on the field (NGS participation data). |  |

---

## Documented fields (nflfastR official dictionary)

In dictionary source order (roughly: game/situation → play description → scoring/probability models → EPA/WPA → binary event flags → player-attribution fields → drive/game metadata → advanced metrics).
...
## Documented but not present in this data pull

`old_game_id` is documented but doesn't appear under that exact name — see `old_game_id_x` / `old_game_id_y` above, an nfl_data_py merge artifact.
