...
def render(season: int, week: int, game_ids: list):
    ...
    if not df.empty:
        filter_mode = st.radio(
            ...
            horizontal=True, key="rb_rec_filter_mode",
        )
        if filter_mode == "Season avg targets/game":
            min_targets = st.number_input(
                "Min avg targets/game", min_value=0.0, max_value=20.0, value=3.0, step=0.5, key="rb_rec_min_targets",
            )
            df = df[df["szn_avg_targets"] >= min_targets]
        else:
            min_receptions = st.number_input(
                "Min avg receptions/game", min_value=0.0, max_value=15.0, value=2.0, step=0.5, key="rb_rec_min_receptions",
            )
